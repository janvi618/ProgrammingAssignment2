...
cacheSolve <- function(x) {                     ## Receive makeCacheMatrix from the caller.
  ...
  startmatrix <- x$get()               ## Call the nested function x$get in makeCacheMatrix to obtain the UNinverted matrix with which to start, and assign it to startmatrix.                         
  endmatrix <- solve(startmatrix)   ## Use solve() to invert the startmatrix.  Assign the result to endmatrix.
  x$set_cache_m(endmatrix)             ## Call nested function x$set_cache_m() in makeCacheMatrix to set m in the cache environment to the local non-NULL inverted result in endmatrix
  endmatrix                            ## Evaluate endmatrix so as to return it to caller/console if cache_m is non NULL.
}